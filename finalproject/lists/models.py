...
class JobListing(models.Model):
    ...
    adjusted_timestamp = models.DateTimeField(blank=True, null=True)
    ...
    def subtract_hours(self):
        return self.deadline - timedelta(hours=8)

    def save(self, *args, **kwargs):
        if not self.pk:  # Only on creation
            self.adjusted_timestamp = timezone.now() - timedelta(hours=8)
        super().save(*args, **kwargs)


    @property
    def deadline_passed(self):
        return timezone.now() - timedelta(hours=6) > self.deadline
