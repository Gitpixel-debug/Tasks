...
from django.contrib.auth.models import User # Explicitly import Django's User model
...
def update_progress(request, listing_id):
    # This view is for AJAX/API updates only
    listing = JobListing.objects.get(pk=listing_id)
    json_data_str = request.body.decode('utf-8')
    data = json.loads(json_data_str)
    listing.percentage = data['progress_value']
    listing.save()
    return JsonResponse({'percentage': data['progress_value']})


# Removed the old 'comment' view entirely as it is now in 'listing_detail_view'
